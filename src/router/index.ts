import { createRouter, createWebHashHistory, createWebHistory, RouteRecordRaw, RouteLocationNormalized, NavigationGuardNext, Router } from 'vue-router'
...
// 构造路由表
function fnAddDynamicMenuRoutes(menuList: StoreState.Role[], routes: RouteRecordRaw[]) {
		...
		if (!isAddDynamicMenuRoutes) {
			try {
				...
				if (res.code == 200) {
					...
					// 本地存储按钮权限集合
					localStorage.setItem('permissions', JSON.stringify(menu.permissions || '[]'))
				...
				console.log(`%c${error} 请求菜单列表和权限失败，跳转至登录页！！`, 'color:orange')
			}
		} else {
			if (to.path == '/' || to.path == '/login') {
				next(from)
